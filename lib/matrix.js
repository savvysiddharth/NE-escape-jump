class Matrix
{
	...
	multiply(n)
	{
		if(n instanceof Matrix) //hadamard multiplication (corresponding element multiplies to this matrix)
		{
			...
			{
				console.error("element wise multiplication failed bcoz of size mismatch!");
	...
	{
		if(matx1.cols == matx2.rows && matx1 instanceof Matrix && matx2 instanceof Matrix)
		{
		...
		{	
			...
			else
			{
				...
				if(!(matx1 instanceof Matrix && matx2 instanceof Matrix))
					console.log('argument to matrix multiplication func should be only Matrix objects');
	...
	static map(matrix,func) //returns mapped matrix obj
	{
	...
	randomize() //randomizes this matrix
	{
		for (let i = 0; i < this.rows; i++)
		{
			for(let j=0; j <this.cols; j++)
	...
	static fromArray(arr) //returns 1d matrix from 1d array
	{
		let rows = arr.length;
		let cols = 1;
